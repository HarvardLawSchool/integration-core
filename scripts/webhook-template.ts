// Import directly from relative paths to avoid circular dependency with mod.ts
import { getParam } from "../src/util/aws-params.ts";
import { generateHMAC } from "../src/util/hmac.ts";
import { getStack, root } from "./_utils.ts";

export async function writeWebhookTemplate(): Promise<void> {
  ...
  if (!webhookSecret) {
    throw new Error("WEBHOOK_SECRET not found in config blob");
  }
